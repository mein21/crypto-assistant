...
ЦЕЛЬ: каждая сделка должна иметь RR ≥ 1.5. Сначала ставь TP к ближайшему сопротивлению (LONG) / поддержке (SHORT) и SL за структурным экстремумом, чтобы RR вышел ≥ 1.5. Если сетап не позволяет — допускается ниже, но в reason объясни почему (например «TP упирается в R=$..., RR=1.1»).
