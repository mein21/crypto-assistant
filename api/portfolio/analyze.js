...
module.exports = async (req, res) => {
    ...
    try {
        ...
        const t1 = Date.now();
        const indicatorsMap = {};
        const indicators4hMap = {};
        // News fetch runs in parallel with the indicator computations so it
        // never extends total latency beyond what indicators already need.
        const newsPromise = fetchLatestNews();
        ...
        const indicatorLines = Object.entries(indicatorsMap)
            .map(([coin, bundle]) => {
                ...
                const line1h = formatIndicatorLine(coin, bundle, livePrice);
                const h4 = indicators4hMap[coin];
                let line4h = '';
                if (h4) {
                    const parts = [`тренд=${h4.trend || 'neutral'}`];
                    if (Number.isFinite(h4.rsi)) parts.push(`RSI=${h4.rsi.toFixed(1)}`);
                    if (h4.macd) parts.push(`MACD ${h4.macd.histogram >= 0 ? 'бычья' : 'медвежья'}`);
                    if (h4.signalScore) parts.push(`БАЛЛ=${h4.signalScore.score > 0 ? '+' : ''}${h4.signalScore.score}`);
                    line4h = `\n  4h: ${parts.join(', ')}`;
                }
                return line1h + line4h;
            }).join('\n') || 'нет данных';
        ...
        const focusSymbols = [...indicatorTargets];
        const newsBlock = formatNewsBlock(news, focusSymbols);
        const newsSection = newsBlock
            ? `\nНовостной фон (последние 24ч, ★ — монеты из портфеля):\n${newsBlock}\n`
...
Технические индикаторы (1h основной + 4h старший ТФ) — RSI, MACD, EMA20/50/200, Bollinger %B, Stochastic %K, ATR, тренд, S/R, объём, предвычисленный БАЛЛ + 8 свечей:
${indicatorLines}
${newsSection}
Предстоящие события: ${upcoming}

ГЛАВНОЕ ПРАВИЛО: направление позиции должно совпадать с трендом 4h. Если позиция LONG, а 4h тренд bearish — рекомендуй закрыть или сильно сузить SL.
Опирайся на согласование 1h и 4h таймфреймов + минимум 3 индикаторов. Учитывай предвычисленный БАЛЛ (положительный = бычий сигнал, отрицательный = медвежий). Учитывай объём и дивергенции. Новости: негатив → отметь в weaknesses, позитив → в strengths.
...
Учти комиссии Bybit: round-trip taker fee = 0.11% (0.055% × 2). TP ближе 0.11% от текущей цены — гарантированный убыток после комиссии. Рекомендуй TP минимум на 0.2% от текущей цены, иначе сделка нерентабельна даже при достижении цели.
